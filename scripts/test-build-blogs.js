...
console.log('Total new blogs prepared:', newBlogs.length);
newBlogs.forEach(b => {
  const raw = getRangeText(b.startLine, b.endLine);
  const blocks = parseParagraphs(raw);
  console.log(`- "${b.title}" -> ${blocks.length} content blocks, slug: ${b.slug}`);
});
