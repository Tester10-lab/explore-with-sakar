...
export const metadata: Metadata = {
  ...
  description: '100% tailor-made Nepal itineraries for solo travelers, couples, and multi-generational families. Designed around your dates, pace, and passions.',
};

export default async function CustomJourneysServicePage() {
  const liveService = await getLiveServiceBySlug('custom-journeys');
  const steps = [
    {
      num: '01',
  ...
  return (
    <div className="min-h-screen bg-parchment-100">
      {/* 1. Page Hero */}
      <PageHero
        badge={liveService?.category || 'Bespoke Private Travel'}
        nepaliTitle={liveService?.nepaliTitle || 'निजी तथा व्यक्तिगत यात्रा योजना'}
        title={liveService?.title || 'Tailor-Made Journeys in Nepal'}
        subtitle={liveService?.subtitle || liveService?.description || 'Every traveler is unique. We craft 100% private, customized itineraries shaped around your personal rhythm, travel dates, and cultural curiosity.'}
        backgroundImage={liveService?.image || '/explore-with-sakar/images/mountains/sunrise-himalayas.jpg'}
        breadcrumbs={[
          { label: 'Services', href: '/services' },
          { label: liveService?.title || 'Custom Journeys' },
        ]}
      />
