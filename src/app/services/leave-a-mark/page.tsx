...
export const metadata: Metadata = {
  title: 'Leave a Mark | Regenerative Travel & Community Conservation with Sakar',
  description:
    'Travel that gives back more than it takes. Engage in community-led conservation, village school mentorship, native reforestation, and ethical wildlife stewardship in Nepal.',
};

export default async function LeaveAMarkServicePage() {
  const liveService = await getLiveServiceBySlug('leave-a-mark');
  const leaveAMarkExperiences = EXPERIENCES.filter(
    (e) => e.category === 'leave-a-mark' || e.category === 'responsible'
  );
  ...
  return (
    <div className="min-h-screen bg-parchment-100">
      {/* 1. Hero Section */}
      <PageHero
        badge={liveService?.badge || 'Signature Travel Pillar'}
        nepaliTitle={liveService?.nepaliTitle || 'सकारात्मक प्रभाव र दिगो संरक्षण'}
        title={liveService?.title || 'Leave a Mark: Travel That Gives Back'}
        subtitle={
          liveService?.subtitle ||
          liveService?.shortDescription ||
          'True travel is not just about what you take away in memories and photos—it is about the dignity, nourishment, and regenerative footprint you leave behind in the communities that open their hearts to you.'
        }
        backgroundImage={liveService?.image || '/explore-with-sakar/images/trails/river-gorge.jpg'}
        breadcrumbs={[
      ...
      <CTASection
        ...
        primaryButtonHref="/services/custom-journeys"
        secondaryButtonText="Browse Curated Experiences"
